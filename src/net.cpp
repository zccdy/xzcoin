...
{
	LogPrintf("<<< ThreadGetMyExternalIP() \n");
    CNetAddr addrLocalHost;
    if (GetMyExternalIP(addrLocalHost))
    {
        LogPrintf("ThreadGetMyExternalIP() returned %s\n", addrLocalHost.ToStringIP());
        AddLocal(addrLocalHost, LOCAL_HTTP);
    }

	LogPrintf("<<< ThreadGetMyExternalIP()  2 \n");
}
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (xbhcoin users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "xbhcoin " + FormatFullVersion();
...
{
	LogPrintf("<<<<<ThreadOpenConnections() \n");
    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            nTries++;
            if (nTries > 100){
					 LogPrintf("<<<<1349 ThreadOpenConnections  .\n");
                	break;
            	}
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. xbhcoin Core Daemon is probably already running."), addrBind.ToString());
        else
