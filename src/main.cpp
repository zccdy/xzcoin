...
#if defined(NDEBUG)
# error "xbhcoin cannot be compiled without assertions."
#endif
...
static const int64_t nTargetTimespan = 2 * 60; // Á½·ÖÖÓµ÷ÕûÒ»´Î
...
const string strMessageMagic = "xbhcoin Signed Message:\n";
...
{
    int64_t nSubsidy = nDefaultSubsidy * COIN;
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits)
{
    ...
    if (hash > bnTarget.getuint256()){
		printf("<<<<<<<<< %s  > %s",hash.GetHex().c_str(),bnTarget.getuint256().GetHex().c_str());
