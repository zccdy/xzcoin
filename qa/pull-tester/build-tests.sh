...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /root/xbhcoin/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /root/xbhcoin/win32-build/src/test/test_bitcoin.exe $OUT_DIR/test_bitcoin.exe
  cp /root/xbhcoin/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /root/xbhcoin/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /root/xbhcoin/linux-build/src/test/test_bitcoin $OUT_DIR/test_bitcoin
  cp /root/xbhcoin/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /root/xbhcoin/linux-build
make check

# Run RPC integration test on Linux:
/root/xbhcoin/qa/rpc-tests/wallet.sh /root/xbhcoin/linux-build/src

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /root/xbhcoin/win32-build
  make check
fi
